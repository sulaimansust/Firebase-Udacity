...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func configureDatabase() {
        ...
        _refHandle = ref.child("messages").observe(.childAdded) {
            ...
            self.messagesTable.insertRows(at: [IndexPath.init(row: self.messages.count-1, section: 0)], with: .automatic)
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // dequeue cell
        let snapshot = self.messages[indexPath.row]
        let cell: UITableViewCell! = messagesTable.dequeueReusableCell(withIdentifier: "messageCell", for: indexPath)
        
        if let dict = snapshot.value as? [String:String] {
            
            let name = dict[Constants.MessageFields.name] ?? "[username]"
            let text = dict[Constants.MessageFields.text] ?? "[text]"
